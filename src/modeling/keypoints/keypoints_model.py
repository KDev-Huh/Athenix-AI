"""
Ultralytics YOLO pose 모델 학습 / 평가 / 저장 / 로드 클래스.
"""
...
from .config import KeypointsConfig


class KeypointsModel:
    """
    YOLO pose 모델을 학습하고 관리하는 클래스.

    사용 예시:
        # 기본 설정으로 학습
        model = KeypointsModel(model_dir="models/keypoints")
        model.train(dataset_yaml="C:/Datasets/.../unified_output/dataset.yaml")
        print(model.evaluate(dataset_yaml=..., split="val"))
        model.save(name="soccernet_keypoints_v1")
        # → models/keypoints/soccernet_keypoints_v1.pt
        # → models/keypoints/soccernet_keypoints_v1_config.json

        # 커스텀 설정으로 학습
        from src.modeling.keypoints.config import KeypointsConfig
        config = KeypointsConfig(
            model_type="yolo11n-pose.pt",
            yolo_params={"epochs": 200, "imgsz": 1280},
        )
        model = KeypointsModel(config=config)
    """

    def __init__(
        self,
        model_dir: str = "models/keypoints",
        config: KeypointsConfig = None,
    ):
        """
        Args:
            model_dir: 모델 가중치 / config 저장 경로
            config:    KeypointsConfig 인스턴스. None이면 기본 설정 사용.
        """
        self.model_dir = model_dir
        os.makedirs(model_dir, exist_ok=True)

        self.config              = config or KeypointsConfig()
        self.model               = None
    ...
    def train(self, dataset_yaml: str) -> None:
        ...
        Args:
            dataset_yaml: Ultralytics dataset.yaml 파일 경로
        """
    ...
    def save(self, name: str = None) -> None:
        ...
        Args:
            name: 파일 이름 접두어. None이면 타임스탬프로 자동 생성.
                  예) "soccernet_v1" → soccernet_v1.pt
                                        soccernet_v1_config.json
        """
        if self.model is None or self._best_weights_path is None:
            raise RuntimeError("저장할 모델이 없습니다. 먼저 train()을 호출해 주세요.")

        prefix = name if name else f"keypoints_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def load(self, weights_path: str, config_path: str = None) -> None:
        ...
        if os.path.exists(resolved_config):
            self.config = KeypointsConfig.load(resolved_config)
