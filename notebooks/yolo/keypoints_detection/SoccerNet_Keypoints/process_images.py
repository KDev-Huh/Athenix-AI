...
def process_unified_soccernet_dataset() -> None:
    ...
    images_dir = base_output_dir / 'images'           # Ultralytics image folders
    yolo_labels_dir = base_output_dir / 'labels'      # Ultralytics label folders
    ...
    for dataset_type in ['train', 'test', 'valid']:
        split_dir = calibration_dir / dataset_type
        if not split_dir.exists():
            continue

        print(f"\n📁 Processing {dataset_type} dataset...")
        annotations_path = split_dir
        ...
        for json_file in tqdm.tqdm(json_files, desc=f"Processing {dataset_type}"):
            ...
            # 5. Copy original image to images/ folder (for YOLO training)
            shutil.copy2(str(image_path), str(images_dir / dataset_type / image_path.name))
...
def create_yolo_dataset_config(base_output_dir: Path) -> None:
    ...
    """
    path_str = str(base_output_dir.absolute()).replace('\\', '/')
...
# 29 keypoints, each with (x, y, visibility)
kpt_shape: [29, 3]
